...
impl ProcCollector {
    ...
    pub fn collect(
            ...
            }
        {
            ...
            if self.processes.contains_key(c_pid) {
                let current_process: HashMap<Pid, HashMap<String, ProcessInfo>> =
                    &self.processes[c_pid];
                ...
                self.details["cpu_percent".to_owned()] = ProcCollectorDetails::F32(
                    cpu_percent_push
            ...
            self.details["uptime".to_owned()] = match self.details["create_time".to_owned()] {
                ProcCollectorDetails::Duration(d) => {
                    ...
                    self.details["uptime".to_owned()] = if days > 0 {
                        ProcCollectorDetails::String(format!(
                            "{}d {}:{}:{}",
                            days, hours, minutes, seconds
                        ));
                    } else {
                        ProcCollectorDetails::String(format!("{}:{}:{}", hours, minutes, seconds));
                    }
                }
            ...
            if self.expand > 0 {
                if self.expand > 1 {
                    self.details["nice"] = self.details["nice"].to_string();
                }
                if SYSTEM == "BSD".to_owned() {
                    if self.expand > 2 {
                        // TODO : Once implemented by psutil
                        self.details["io_read".to_owned()] =
                            ProcCollectorDetails::String("?".to_owned());
                        if self.expand > 3 {
                            self.details["io_write".to_owned()] =
                                ProcCollectorDetails::String("?".to_owned());
                        }
                    }
                } else {
                ...
                if self.expand > 4 {
                    self.details["terminal".to_owned()] = ProcCollectorDetails::String(
            ...
            self.details_cpu
                .push(match self.details["cpu_percent".to_owned()] {
                    ProcCollectorDetails::String(s) => s.parse::<u32>(),
                    _ => {
            ...
            let mut mem: f32 = match self.details["memory_percent".to_owned()] {
                ...
                _ => {
                    errlog("Malformed memory_percent");
                    0.0
                }
            };
    ...
    pub fn tree(
        ...
        let mut out :HashMap<String, HashMap<String,ProcCollectorDetails>> = HashMap::<String, HashMap<String,ProcCollectorDetails>>::new();
        let mut err: f32 = 0.0;
        let mut det_cpu: f64 = 0.0;
        let mut infolist: HashMap<String, HashMap<String, ProcCollectorDetails>> =
            HashMap::<String, HashMap<String, ProcCollectorDetails>>::new();
        self.tree_counter += 1;
            ...
            match p.ppid() {
                Ok(o) => {
                    match o {
                        Some(pid) => {
                            match tree.get(&pid) {
                                Ok(&mut v) => v.push(p.pid()),
                                Err(e) => tree.insert(&pid, vec![p.pid()]),
                            }
        ...
        if tree.contains_key(0) && tree.get(0).contains(0) {
            tree.get_mut(0).unwrap().remove_item(0);
        }

        if tree.len() > 0 {
            ProcCollector::create_tree(self, tree.iter().min().unwrap(), tree, String::default(), String::default(), false, 0, None, &mut infolist, &mut proc_per_cpu, &mut search, &mut out, &mut det_cpu, CONFIG)
        }
    ...
    pub fn create_tree(
        ...
        infolist: &mut HashMap<String, HashMap<String, ProcCollectorDetails>>,
        proc_per_cpu: &mut bool,
        search: &mut String,
        out: &mut HashMap<String, HashMap<String,ProcCollectorDetails>>,
        det_cpu: &mut f64,
        CONFIG : &mut Config,
    ) {
        ...
        if search && !found {
            if self.detailed && pid == self.detailed_pid {
                det_cpu = match getinfo.get(&"cpu_percent".to_owned()) {
                    Ok(p) => match p {
                        ProcCollectorDetails::F64(f) => f,
                        _ => {
                            errlog("Malformed information found in cpu_percent");
                            0.0
                        }
                    },
                    Err(e) => {
        ...
        if cont {
                    ...
                    Some(o) => match o {
                        ProcCollectorDetails::F32(f) => {
                            if proc_per_cpu {
                                ...
                                format!("{:.2}", f / THREADS as f32).parse::<f32>();
                            }
                        },
                        _ => {
                    ...
                    Some(o) => match o {
                        ProcCollectorDetails::String(s) => {
                            ...
                            if ws.len() > 0 {
                                ...
                                ("[".to_owned() + match getinfo.get(&"name".to_owned()) {
                                    Some(p) => match p {
                                        ProcCollectorDetails::String(s) => s.clone().as_str(),
                                        _ => {
                                            errlog("Malformed type in getinfo['name']");
                                            ""
                                        },
                                    },
                                    None => "",
                ...
                if CONFIG.proc_mem_bytes {
                    mem_b = match getinfo.get(&"memory_info".to_owned()) {
                        Some(p) => match p {
                            ...
                            _ => {
                                errlog("Malformed type in getinfo['memory_info']");
                                0
                            }
                        },
                        None => 0,
            ...
            match collapse_to {
                Some(u) => if search.len() == 0 {
                    out[u as usize]["threads".to_owned()] = match out[u as usize]["threads".to_owned()] {
                    ...
                    out[u as usize]["mem".to_owned()] = match out[u as usize]["mem".to_owned()] {
                    ...
                    out[u as usize]["mem_b".to_owned()] = match out[u as usize]["mem_b".to_owned()] {
            ...
            if elser {
                if tree.contains_key(&pid) && tree.get(&pid).unwrap().len() > 0 {
                    let sign : &str = if collapse {
                        "+"
                    } else {
                        "-"
                    };
                    inindent = inindent.replace(" ├─ ", "[".to_owned() + sign + "]─");
                }
                out.insert(
                    pid, 
                    vec![
                        ...
                        ("cmd", ProcCollectorDetails::String(cmd.replace("\n", "").replace("\t", "").replace("\\", ""))),
        ...
        for child in children {
            ProcCollector::create_tree(self, child, tree, indent + " | ", indent + " ├─ ", found, depth + 1, collapse_to, infolist, proc_per_cpu, search, out, det_cpu, CONFIG)
        }
        ProcCollector::create_tree(self, tree[pid][tree[pid].len() - 2], tree, indent + " ", indent + " └─ ", false, depth + 1, collapse_to, infolist, proc_per_cpu, search, out, det_cpu, CONFIG);
    }
