...
impl NetBox {
    ...
    pub fn calc_size(&mut self, term: &OnceCell<Mutex<Term>>, b_cpu_h: i32, b_mem_h: i32) {
        ...
        self.set_parent_width(
            ((term.get().unwrap().try_lock().unwrap().get_width() as u32) * width_p / 100) as u32,
        );
        self.set_parent_height(
            u32::try_from(
                term.get().unwrap().try_lock().unwrap().get_height() as i32 - b_cpu_h - b_mem_h,
            )
            .unwrap_or(0),
        );
        self.set_parent_y(
            u32::try_from(
                (term.get().unwrap().try_lock().unwrap().get_height() as i32)
    ...
    pub fn draw_bg(
        ...
        format!(
            ...
            create_box(
                ...
                None,
                None,
                Some(theme.get().unwrap().try_lock().unwrap().colors.net_box),
                None,
                true,
                Some(Boxes::NetBox),
                term,
    ...
    pub fn draw_fg(
        ...
        if net.get().unwrap().try_lock().unwrap().get_parent().get_redraw() {
            self.redraw = true;
        }
        if net.get().unwrap().try_lock().unwrap().nic.is_none() {
            return;
        }
        ...
        let reset: bool = match net
            ...
            NetCollectorStat::String(s) => {
                errlog(format!(
                    "Malformed type in net.get().unwrap().try_lock().unwrap().stats[{}]['download']['offset']",
                    nic_name
                ));
        ...
        if self.get_parent().get_resized() || self.get_redraw() {
            ...
            out_misc.push_str(
                format!(
                    ...
                    theme.get().unwrap().try_lock().unwrap().colors.net_box,
            ...
            if (w as usize) - nic_name[..10].len() - 20 > 6 {
                ...
                out_misc.push_str(
                    format!(
                        ...
                        if net.get().unwrap().try_lock().unwrap().get_auto_min() {
                            fx::b
                        } else {
                            ""
                        },
            ...
            .collect::<Vec<String>>()
        {
            ...
            if self.get_redraw() {
                stats.insert("redraw".to_owned(), NetCollectorStat::Bool(true));
            }
            if match stats.get(&"redraw".to_owned()).unwrap() {
                NetCollectorStat::Bool(b) => b.to_owned(),
                NetCollectorStat::I32(i) => i.to_owned() > 0,
                NetCollectorStat::Vec(v) => v.to_owned().len() > 0,
                NetCollectorStat::U64(u) => u.to_owned() > 0,
                NetCollectorStat::String(s) => {
                    errlog("Malformed type in stats['redraw']".to_owned());
                    s.to_owned().parse::<i32>().unwrap_or(0) > 0
                }
            } || self.get_parent().get_resized()
            {
                graphs.get().unwrap().try_lock().unwrap().net.insert(
                    direction.clone(),
                    Graph::new_with_vec::<Color>(
                        ...
                        theme
                            ...
                            .unwrap()
                            .to_owned(),
                        match stats.get(&"speed".to_owned()).unwrap() {
                            NetCollectorStat::Vec(v) => {
                                v.iter().map(|u| u.to_owned() as i32).collect()
                            }
                            _ => vec![],
                        },
                        term,
                        ...
                        if CONFIG.get().unwrap().try_lock().unwrap().net_sync {
                            net.get().unwrap().try_lock().unwrap().get_sync_top()
                        } else {
                            match stats.get(&"graph_top".to_owned()).unwrap() {
                                ...
                                NetCollectorStat::String(s) => {
                                    ...
                                    s.to_owned().parse::<i32>().unwrap_or(0)
                                }
                            }
                        },
                        0,
                        if CONFIG.get().unwrap().try_lock().unwrap().net_color_fixed {
                            Some(
                                net.get()
                                    .unwrap()
                                    .try_lock()
                    ...
                    graphs
                        ...
                        .call(
                            if match stats.get(&"redraw".to_owned()).unwrap() {
                                ...
                                Some(match stats.get(&"speed".to_owned()).unwrap() {
            ...
            net.get().unwrap().try_lock().unwrap().set_strings_inner_index(
                nic_name.clone(),
                direction.clone(),
                strings.clone(),
            );
            net.get().unwrap().try_lock().unwrap().set_stats_inner_index(
                nic_name.clone(),
                direction.clone(),
                stats.clone(),
            );
        ...
        out.push_str(
            format!(
                "{}{}{}{}",
                mv::to(y, x),
                theme.get().unwrap().try_lock().unwrap().colors.graph_text.call(
                    if CONFIG.get().unwrap().try_lock().unwrap().net_sync {
                        net.get().unwrap().try_lock().unwrap().get_sync_string()
                    } else {
                        net.get()
                            .unwrap()
                            .try_lock()
