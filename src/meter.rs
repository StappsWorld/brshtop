...
impl Meter {
        ...
        match value {
            Some(i) => {
                ...
                if self.saved.contains_key(&new_val) {
                    self.out = self.saved[&new_val].clone();
                } else {
    ...
    pub fn _create(&mut self, value: i32, term: &OnceCell<Mutex<Term>>) -> String {
        ...
        for i in 1..self.width + 1 {
            if value >= (i * 100 / self.width) as i32 {
                out.push_str(
                    format!(
                        "{}{}",
                        self.color_gradient[if !self.invert {
                ...
                out.push_str(
                    self.color_inactive
                        .call(symbol::meter.repeat((self.width + 1 - i) as usize), term)
                        .to_string()
                        .as_str(),
                );
        ...
        if self.saved.contains_key(&new_value) {
            self.saved.insert(new_value.clone(), out.clone());
        }
